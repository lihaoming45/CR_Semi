...
from model.PointRend2.sampling_point2 import sampling_points, point_sample
    ...
    def inference(self, x, p2_1, p2_2, out):
        ...
        while out.shape[-1] != x.shape[-1]:
            ...
            if out.dtype==torch.float:
                out = (out.reshape(B, C, -1).half().scatter_(2, points_idx, rend).view(B, C, D, H, W))
            else:
                out = (out.reshape(B, C, -1) .scatter_(2, points_idx, rend).view(B, C, D, H, W))
...
class PointRend(nn.Module):
    def __init__(self, backbone, head,ema=False):
        super().__init__()
