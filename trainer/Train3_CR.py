...
from model.PointRend2.pointrend3_CR import PointRend, PointHead
from model.PointRend2.sampling_point2 import point_sample, constrastive_learning
...
from model.PointRend2.generator3_CR import train_val_loader  # spatial size进行了resize，再扣patch
from model.PointRend2.generator3_CR import create_list_MYGE
...
@click.option('--test-path', type=str, default=None, help='Path to dataset folder')
@click.option('--models-path', type=str, default="/home/lhm/3D_Unet/model/PointRend2/snapshots/snapshot_Train3_CR",
...
@click.option('--milestones', type=str, default='50', help='Milestones for LR decreasing')
...
def train(data_path, test_path, models_path, snapshot, batch_size, epochs, start_lr, milestones, gpu,
    ...
    writer = SummaryWriter(log_dir='logs_tensorboardX_Train3_CR', comment='loss')
    ...
    for epoch in range(starting_epoch, starting_epoch + epochs):
        ...
        for x, y, y_cr, idx in train_iterator:
            ...
            y_cr = y_cr.to(0, non_blocking=True)
            ...
            if epoch>12:
                cl_loss = constrastive_learning(result["coarse"], result['p2_1'], result['p2_2'], y_cr, is_F=True,
                                                is_O=True)
        ...
        if epoch % 5 == 0:
            ...
            if np.mean(testEpo_eval[0]) > 0.80:
                torch.save(net.state_dict(), os.path.join(models_path, '_'.join(["UnetCR_3D", str(epoch + 1)])))
                # if epoch >= 50:
